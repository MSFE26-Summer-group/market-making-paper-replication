...
class TestModel:
    ...
    def test_actor_critic_shapes_and_bounds(self) -> None:
        model = ActorCritic(n_features=40, n_aux=2, d_model=32)
        dist, value = model(torch.randn(4, 10, 40), torch.randn(4, 2))
    ...
    def test_act_returns_logp_and_value(self) -> None:
        model = ActorCritic(n_features=40, n_aux=2, d_model=32)
        action, logp, value = model.act(torch.randn(1, 10, 40), torch.randn(1, 2))
...
class TestPPO:
    def test_rollout_and_update_run(self) -> None:
        ...
        model = ActorCritic(n_features=40, n_aux=2, d_model=16)
